...
# --- GUÍA DE REFERENCIA ---
with st.expander("📖 Manual de Acecho (RSI & MACD)"):
    st.markdown("""
    ### 📉 Indicadores de Decisión
    * **RSI Anual (50):** Mide la fuerza relativa de los últimos 12 meses. 
        * **Zona de Acecho:** Cuando el RSI está cerca o por debajo de 45 en empresas de alta calidad.
    * **MACD (Inercia):** * **Histograma:** Nos dice si la caída está perdiendo fuerza (barras rojas más cortas) o si el impulso alcista inicia (barras verdes).
    * **Distancia al Suelo (52W Low %):** * Cuanto más cerca de **0%**, más cerca estamos del precio más bajo pagado en todo el año.
    """)
...
def fetch_full_data(ticker):
    try:
        ...
        # Cálculos Técnicos
        hist['RSI_50'] = ta.rsi(hist['Close'], length=50) # RSI de Largo Plazo
...
if data_list:
    ...
    # Mostrar tabla resumen sin la columna del dataframe histórico
    st.dataframe(pd.DataFrame(data_list).drop(columns=['df']), use_container_width=True)

    st.divider()
    seleccion = st.selectbox("🎯 Selección de Activo para Análisis de Acecho:", [d["Ticker"] for d in data_list])

    if seleccion:
        item = next(i for i in data_list if i["Ticker"] == seleccion)
        df_p = item["df"].tail(252) # Mostrar último año de datos
        last_p = df_p.iloc[-1]

        # --- CONFIGURACIÓN DE GRÁFICA: 2 NIVELES ---
        fig = make_subplots(
            rows=2, cols=1, 
            shared_xaxes=True, 
            vertical_spacing=0.1,
            row_heights=[0.6, 0.4],
            subplot_titles=("ÍNDICE DE FUERZA RELATIVA (RSI ANUAL)", "INERCIA Y MOMENTUM (MACD)")
        )
        
        # Función para etiquetas flotantes
        ...
        add_lab(fig, last_p['MACD_12_26_9'], f"MACD: {item['MACD']}", "#2962ff", 2, 1, 15)
        ...
        st.info(f"💡 **Estrategia:** Estas analizando {seleccion}. El RSI Anual de {item['RSI Anual']} indica la posición de la empresa respecto a su fuerza histórica del último año.")

else:
    st.warning("No se encontraron empresas que cumplan con los filtros de ingeniería actuales. Intenta ajustar el Market Cap o el límite de RSI.")
