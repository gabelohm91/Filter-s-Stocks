...
def get_all_tickers():
    ...
    # 2. Selección de Valor, ETFs y Prospectos
    especiales = [
...
def fetch_full_data(ticker):
    try:
        ...
        de_ratio = info.get('debtToEquity', 0) or 0  # FIX: None → 0

        # FIX: Calcular variación 52 semanas
        price_52w_ago = hist['Close'].iloc[-252] if len(hist) >= 252 else hist['Close'].iloc[0]
        change_52w = round(((last['Close'] - price_52w_ago) / price_52w_ago) * 100, 2)
        ...
        return {
            ...
            "52W Change(%)": change_52w,  # FIX: columna nueva
            "df": hist
        }
    except:
        return None
...
if st.checkbox("🚀 Iniciar Escaneo Profundo (S&P 500 + Especiales)"):
    ...
    for i, t in enumerate(all_tickers):
        ...
        if res:
            # FIX: ETFs con D/E = 0 pasan el filtro correctamente
            de_ok = (res["D/E Ratio(%)"] == 0) or (res["D/E Ratio(%)"] <= max_de_ratio)
...
if data_list:
    ...
    # FIX: key para persistir selección en session_state
    ...
    if seleccion:
        ...
        # FIX: add_annotation con xref/yref correctos para subplots
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='rgba(173,216,230,0.2)', width=1), name="B.Sup"
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df_p.index, y=df_p[col_bbl],
            line=dict(color='rgba(173,216,230,0.2)', width=1),
            fill='tonexty', name="B.Inf"
        ), row=1, col=1)

        add_lab(fig, last_p['Close'], f" PRECIO: ${item['Precio']}", "white", 1, offset=0)
        add_lab(fig, last_p[col_bbu], f" BS: {round(last_p[col_bbu], 2)}", "lightblue", 1, offset=0)
        add_lab(fig, last_p[col_bbl], f" BI: {round(last_p[col_bbl], 2)}", "lightblue", 1, offset=0)
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=h_colors, name="Hist"
        ), row=3, col=1)
        fig.add_trace(go.Scatter(
        ...
        add_lab(fig, last_p['MACD_12_26_9'], f" M: {item['MACD']}", "#2962ff", 3, offset=0)
        add_lab(fig, last_p['MACDs_12_26_9'], f" S: {item['Señal']}", "#ff6d00", 3, offset=0)
